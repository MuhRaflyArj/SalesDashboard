...
# Body jenis visualisasi data per-SPBU
def body_spbu(df) :
    ...
    # Mengambil data tren penjualan berdasarkan SPBU 
    # dan nilai yang dipilih dengan aggreagate tertentu
    ...
    with col1_1 :
        ...
        # Menulis total nilai pada kolom kedua
        with col1_1_2 :
            st.write("**Total**") # Nama kolom
            for total in total_sales_dict.values() :
                st.write("{:,.0f}".format(total))
        
    # Isi kolom untuk grafik tren penjualan suatu produk pada SPBU
    with col1_2 :
    ...
    # Isi kolom perbandingan penjualan setiap produk
    with col2_3 :
        # Buat barchart untuk perbandingan setiap produk
...
def body_region(df) :
    ...
    # Mengambil data untuk perbandingan produk PSO dan standar dari region yang dipilih
...
def body_keseluruhan(df) :
    ...
    # Mengambil data tren penjualan keseluruhan ebrdasarkan produk yang dipilih
    ...
    # Mengambil data penualan setiap produk (key = nama produk, value = total penjualan)
    ...
    with col2_1 :
        ...
        fig_pso_barchart = graph.sales_barchart(
            total_pso, 
            header_data["selected value"], 
            f"Perbandingan {header_data['selected value']} PSO dan Standard"
        )
    ...
    with col2_2 :
        # Buat sales barchart untuk perbandingan penjualan antar produk
        fig_sales_barchart = graph.sales_barchart(
            sales_numbers, 
            header_data["selected value"], 
