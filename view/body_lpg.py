...
def body_sh(df) :
    ...
    with col1_2 :
        sales_chart = graph.sales_chart_sh(sales_data, header_data["selected product"], header_data["selected value"])
        st.plotly_chart(sales_chart, use_column_width=True)
...
def body_keseluruhan(df) :
    ...
    with col1_1 :
        ...
        fig_pso_barchart = graph.sales_barchart(total_pso, header_data["selected value"], f"Perbandingan {header_data['selected value']} PSO dan Standar")
        st.plotly_chart(fig_pso_barchart, use_container_width=True)
